package fr.launcher.ImHere.result;
...
import fr.launcher.ImHere.pojo.AppPojo;
import fr.launcher.ImHere.pojo.Pojo;
import fr.launcher.ImHere.utils.SpaceTokenizer;
...
public class AppResult extends Result {
    ...
    protected PopupMenu buildPopupMenu(Context context, final RecordAdapter parent, View parentView) {
        ...
        if (ImHereApplication.getRootHandler(context).isRootActivated() && ImHereApplication.getRootHandler(context).isRootAvailable()) {
    ...
    private void excludeFromAppList(Context context, AppPojo appPojo) {
        ImHereApplication.getDataHandler(context).addToExcluded(appPojo.packageName);
        //remove app pojo from appProvider results - no need to reset handler
        ImHereApplication.getDataHandler(context).getAppProvider().removeApp(appPojo);
        ImHereApplication.getDataHandler(context).removeFromFavorites((MainActivity) context, appPojo.id);
    ...
    private void launchEditTagsDialog(final Context context, final Pojo app) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(context,
                android.R.layout.simple_dropdown_item_1line, ImHereApplication.getDataHandler(context).getTagsHandler().getAllTagsAsArray());
        ...
        builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ImHereApplication.getDataHandler(context).getTagsHandler().setTags(app.id, tagInput.getText().toString());
    ...
    private void hibernate(Context context, AppPojo app) {
        ...
        if (!ImHereApplication.getRootHandler(context).hibernateApp(appPojo.packageName)) {
    ...
    public Drawable getDrawable(Context context) {
        ...
        if (icon == null) {
             icon = ImHereApplication.getIconsHandler(context).getDrawableIconForPackage(className);
        }
