...
import fr.launcher.ImHere.dataprovider.SearchProvider;
...
public class SettingsActivity extends PreferenceActivity implements
    ...
    private void addExcludedAppSettings(final SharedPreferences prefs) {
        ...
        multiPreference.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                ...
                ImHereApplication.getDataHandler(SettingsActivity.this).getAppProvider().reload();
                return false;
            }
        });
        if (!hasExcludedApps(prefs)) {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (key.equalsIgnoreCase("icons-pack")) {
            ImHereApplication.getIconsHandler(this).loadIconsPack(sharedPreferences.getString(key, "default"));
        }

        if (key.equalsIgnoreCase("sort-apps")) {
            // Reload application list
            final AppProvider provider = ImHereApplication.getDataHandler(this).getAppProvider();
            if (provider != null) {
                provider.reload();
            }
        }
    ...
    private void fixSummaries() {
        int historyLength = ImHereApplication.getDataHandler(this).getHistoryLength();
        if (historyLength > 5) {
    ...
    protected void setListPreferenceIconsPacksData(ListPreference lp) {
        IconsHandler iph = ImHereApplication.getIconsHandler(this);
