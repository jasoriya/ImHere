...
public class HistorySearcher extends Searcher {
    ...
    protected List<Pojo> doInBackground(Void... voids) {
        ...
        if(excludeFavorites){
            favoritesPojo = ImHereApplication.getDataHandler(activity).getFavorites(activity.tryToRetrieve);
        }

        return ImHereApplication.getDataHandler(activity).getHistory(activity, MAX_RECORDS, smartHistory, favoritesPojo);
    }
}
